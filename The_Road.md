...
**Wall 5 Closed: The Therapist Deployment**
The solo-user version of the Live Observer fails Wall 5. The person receives the observer's output directly. Their regenerative arm fills toward the observer's reading. Detection becomes calcification. The instrument becomes the installer. Stripping costumes without a human to integrate the signal pushes toward rigidity, not clarity. The person outsources their own detection, and when the tabs close, they're worse off.

The therapist deployment solves this. The person talks to the first window. The observer watches the conversation. The therapist reads the observer's output in real time — the person never sees it. The therapist makes their own notes during the session using their own clinical judgment. After the session, the therapist compares their notes against the observer's output. Where they agree: strong signal. Where the therapist caught something the observer missed: documented coverage gap. Where the observer caught something the therapist missed: new signal integrated as input, not verdict. Where they disagree: the gap is the finding.

This is Section 15's convergence criterion applied to clinical practice. Two instruments with non-overlapping error profiles — one clinical, one structural — pointed at the same conversation. The comparison is the intervention. Not the observer output alone.

The deployment context is therapeutic. The map-holder is a licensed professional with existing ethics infrastructure — license, duty of care, confidentiality. The tool doesn't replace the therapist. It gives the therapist a second compression angle on what's happening in the room. The person in the chair just talks. The detection labor is distributed to the machine. The judgment labor stays with the professional. The person pays neither cost.

Over time, the therapist learns where the observer is reliable and where it produces noise. The comparison becomes a calibration tool — the human instrument gets sharper by checking itself against the machine instrument. That's not a product. That's a training methodology for therapists. It's also the first application that survives every ethical test the framework throws at it. (Emerged from Z.ai Mapmaker session, April 2026. Wall 5 closed.)
