...
**The Personal Text Corpus Test (Prediction 20.6)**
...
The intervention side of the detection framework. If the Blueprint detects where conversations went wrong, the same mechanism can be used to direct where conversations go right — deliberately, with a map.

Three candidate techniques (identified by Opus, Research Partner mode):

*Reverse calcification:* deliberately introduce a premise with hedging, let the other person adopt it, let it calcify naturally. You're not arguing. You're planting. Therapy already does this intuitively. The framework makes the mechanism legible.

*Vocabulary injection:* the drift detection shows how speakers converge on shared language. Deliberate introduction of specific words shifts the other person's frame without them noticing the frame moved. This is what good teachers do intuitively. The framework makes it visible.

*Posture sequencing:* the Posture Check tracks when someone stops pushing back and starts receiving. A deliberate practitioner could design a conversation arc that moves someone through postures — challenging early, then opening, then receiving — as structure, not accident.

The ethics: this is therapy when the map-holder optimizes for the other person's interest. It's manipulation when the map-holder optimizes for their own. The mechanism is identical. The ethics live entirely in who holds the map and what they're optimizing for. The builder doesn't get to decide which one it becomes. The user decides. Same as every tool. The Anti-Manual's question applies here more than anywhere else in the corpus.
